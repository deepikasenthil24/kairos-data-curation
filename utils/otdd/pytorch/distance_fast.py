...
class DatasetDistance:
    ...
    def convert_subset_to_tensor(self, s):
        return torch.stack([t for t in s])
    ...
    def distance(self, maxsamples=10000, return_coupling=False):
        ...
        if self.method == "augmentation":
            ...
            Z1 = torch.cat(
                (self.X1[idxs_1], self.convert_subset_to_tensor(self.Y1[idxs_1]).type(self.X1.dtype).unsqueeze(1)), -1
            )
            Z2 = torch.cat(
                (self.X2[idxs_2], self.convert_subset_to_tensor(self.Y2[idxs_2]).type(self.X2.dtype).unsqueeze(1)), -1
            )
